...
const ensureToken = () => {
  ...
  if (!trimmed) {
    throw new ReplicateConfigurationError("Server REPLICATE_API_TOKEN value is empty after trimming.");
  }

  // Replicate's SDK automatically prefixes the supplied token when building the
  // Authorization header. Normalise common copy/paste formats so users can drop
  // in the exact text from docs (e.g. "Authorization: Bearer r8-...").
  const withoutLabel = trimmed.replace(/^authorization:\s*/i, "");
  const withoutPrefix = withoutLabel.replace(/^(token|bearer)\s+/i, "");

  const tokenMatch = withoutPrefix.match(/(r8[-_A-Za-z0-9]{10,})/);
  const normalized = tokenMatch ? tokenMatch[1] : withoutPrefix;

  if (!normalized || /^(token|bearer)\s+/i.test(normalized)) {
    throw new ReplicateConfigurationError("Server REPLICATE_API_TOKEN value is invalid.");
  }
