...
const ensureToken = () => {
  ...
  // Authorization header. Strip any accidental "Token " or "Bearer " prefixes so
  // users can paste the literal value straight from Replicate without double-prefixing.
  const normalized = trimmed.replace(/^(token|bearer)\s+/i, "");
