...
const ensureToken = () => {
  ...
  // Replicate's SDK automatically prefixes the supplied token with "Token ".
  // Strip any accidental "Token " prefix so users can paste the exact string
  // from the dashboard without worrying about duplication.
  const normalized = trimmed.replace(/^token\s+/i, "");
  if (!normalized) {
